jQuery(document).ready(function() {
  ...
  /**
   * Aceasta functie genereaza mesajele de eroare pentru pagina settings.
   */
  jQuery('.settings').submit(function() { 
    var container = this;
    jQuery(container).ajaxSubmit({
      success: function(data){
        var ajaxResult= jQuery('<div/>').html(data), $ajaxContent = ajaxResult.find('#setting-error-settings_updated');
        if($ajaxContent['length'])
          jQuery(container).find('.message').html($ajaxContent).show();
      }, 
      timeout: 5000
    });
    setTimeout("jQuery('.settings').find('.message').hide('slow');", 5000);
    return false; 
  });
});
