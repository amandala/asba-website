...
<ul id="all_filters">
	...
	foreach ( $filters as $slug => $filter ) { ?>
		<li id="tribe_events_filter_<?php echo esc_attr( $slug ); ?>">
			<label><input type="checkbox" id="<?php echo esc_attr( $slug ); ?>" name="tribe_active_filters[]" value="<?php echo esc_attr( $slug ) ; ?>" <?php checked( in_array( $slug, $current_filters ) ); ?> /> <?php echo $filter['name']; ?></label>
		</li>
	<?php }	?>
</ul>
</div>